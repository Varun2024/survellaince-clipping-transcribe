...
def run_pipeline(input_path: str, output_dir: str = "output") -> dict:
    ...
    # Ensure dirs
    frames_dir = output_dir_p / "frames"
    results_json = output_dir_p / "detections.json"
    segments_json = output_dir_p / "segments.json"
    clips_dir = output_dir_p / "clips"
    transcripts_dir = output_dir_p / "transcripts"
    report_path = output_dir_p / "report.txt"
    alerts_path = output_dir_p / "alerts.json"
    ...
    print(f"Starting MVP pipeline for {input_path_p}")
    ...
    # 1) Ingest frames
    ingest_video(str(input_path_p), str(frames_dir), frame_rate=frame_rate)
    ...
    pose_fatigue_json = output_dir_p / "pose_fatigue.json"
    ...
    # 5) Clip segments
    clip_paths = clip_segments(str(input_path_p), str(segments_json), str(clips_dir), fps=frame_rate)
    ...
    qwen_output_dir = output_dir_p / "qwen"
    ...
    return {
        ...
        "pose_fatigue_path": str(pose_fatigue_json),
        "transcripts_index_path": str(index_path),
        "qwen_output_dir": str(qwen_output_dir),
    }
